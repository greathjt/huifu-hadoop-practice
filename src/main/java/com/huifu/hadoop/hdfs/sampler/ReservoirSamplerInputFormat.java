...
public class ReservoirSamplerInputFormat<K extends Writable, V extends Writable> extends InputFormat {
    ...
    public RecordReader createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
        ...
        return new ReservoirSamplerRecordReader<>
                (context, getInputFormat(conf).createRecordReader(split, context), getNumSamples(conf), getMaxRecordsToRead(conf));
    }
    ...
    public static class ReservoirSamplerRecordReader<K extends Writable, V extends Writable> extends RecordReader {
        ...
        public ReservoirSamplerRecordReader(TaskAttemptContext context, RecordReader<K, V> rr, int numSamples, int maxRecords) {
            this.conf = context.getConfiguration();
            this.rr = rr;
            this.numSamples = numSamples;
            this.maxRecords = maxRecords;
            this.keys = Lists.newArrayListWithExpectedSize(numSamples);
            this.values = Lists.newArrayListWithExpectedSize(numSamples);
        }
